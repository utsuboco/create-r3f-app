  ...
  const tsconfig = `
{
  ...
  await fs.writeFile(sysPath.join(root, 'tsconfig.json'), tsconfig, { spaces: 2 });
...
const injectNextStyledConfig = async (root) => {
  ...
  content = content.replace('//   styledComponents: true,', '  styledComponents: true,');
...
const initForStyledComponents = async (root) => {
  ...
  output.info('Downgraded to @types/react@17.0.43 in order to fix styled-components (see DefinitelyTyped/issues/59765)');
  const downgradeReactCmd = useYarn ? ['upgrade', '@types/react@17.0.43'] : ['install', '@types/react@17.0.43'];
