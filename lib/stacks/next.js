...
const migrateTailwindToStyled = async (root, rootFolder) => {
  const indexTailwindCss = await fs.readFile(
    sysPath.join(
      root,
      rootFolder === "src" ? `${rootFolder}/styles/index.css` : `app/global.css`
    )
  );
  ...
  for (const targetJsxFile of targetJsxFiles) {
    ...
    const styleFilePath = [
      ...
      .join("")
      .replace(rootFolder === "src" ? /src/ : /app/, "@")
      .replace(/.jsx/, "");
...
async function next(
  ...
  const rootFolder = projectBranch === "layouts-rfc" ? "app" : "src";
